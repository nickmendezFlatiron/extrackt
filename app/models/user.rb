class User < ApplicationRecord
  ...
  validates :email ,presence: true , uniqueness: {message: "Email is connected to an existing account"} , format: {with: /[a-zA-Z0-9@.]|(^$)/ , message: "may only include alphanumeric characters or @"}
  validates :full_name , presence: true , format: {with: /[a-zA-Z]+\s[a-zA-Z]/ , message: "Names may only use alphabetic characters , separated by one space"}
end
